...
import { ShoppingCart, ArrowLeft, CheckCircle, Smartphone, Plus, Phone, AlertCircle, LogIn, User as UserIcon, LogOut, Shield, Bell, Copy, Upload, Image as ImageIcon } from 'lucide-react';
...
export default function App() {
  ...
  const updateOrder = (key: keyof OrderState, value: any) => {
    setOrder(prev => {
      ...
      if (key === 'boostType' || key === 'quantity') {
        newState.price = calculatePrice(newState.boostType, newState.quantity);
      }
      return newState;
    });
  };
  ...
  const handleCheckoutStart = () => {
    ...
    setPaymentSenderPhone(user?.phone || '');
    setPaymentRefNo('');
  ...
  const handleFinalPurchase = async () => {
    ...
    // Require login for purchase (Optional, but good for linking IDs)
    if (!user) {
      setView('LOGIN');
      return;
    }
    ...
    // Submit to Service (Internal DB Only)
  ...
  const renderWizardStep = () => {
    switch (step) {
      ...
      case 3: 
        const options = order.boostType ? QUANTITY_OPTIONS[order.boostType] : [];
        return (
          <div className="space-y-6">
            <h3 className="text-xl text-center text-slate-300">
            ...
            <div className="bg-slate-900 p-6 rounded-2xl border border-slate-800">
              ...
              <div className="relative">
                <input 
                  ...
                  maxLength={11}
                  disabled={user?.role === 'customer'} // If logged in, maybe lock it or leave editable
                  onChange={(e) => {
    ...
    <div className="min-h-screen font-sans bg-slate-950 text-slate-50 selection:bg-fuchsia-500/30 flex flex-col">
      ...
      <nav className="fixed top-0 w-full z-50 bg-slate-950/80 backdrop-blur-md border-b border-slate-800">
        <div className="container mx-auto px-4 h-16 flex items-center justify-between">
          ...
          >
            <img src="https://image2url.com/r2/default/images/1770547339720-83e35c96-8e65-4d5b-aea2-3f112760363b.png" alt="NethminaOFC" className="h-10 md:h-12 w-auto object-contain" />
            <span className="text-xl font-bold tracking-tighter text-white">NethminaOFC</span>
          </div>
            ...
            {user ? (
              <div className="flex items-center gap-3">
                 <div className="hidden md:flex flex-col items-end text-xs mr-2">
                   ...
                   <span className="font-bold text-violet-400 font-mono tracking-wider">{user.id}</span>
                 </div>
                    ...
                    <div className="bg-slate-800/50 p-4 rounded-xl border border-violet-500/30 mb-6">
                       ...
                       <div className="flex items-center gap-2 bg-slate-950 p-3 rounded-lg border border-slate-700">
                         ...
                         <button 
                           ...
                           className="ml-auto p-2 bg-slate-800 hover:bg-slate-700 rounded-md text-slate-300 transition-colors"
                           title="Copy Number"
                         >
                           <Copy size={16} />
                         </button>
