...
import { Clock, CheckCircle, XCircle, Loader2, ExternalLink, RefreshCw, Bell } from 'lucide-react';
                  ...
                  {order.items.map((item, idx) => (
                    <div key={idx} className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 bg-slate-950/50 rounded-xl border border-slate-800/50">
                      ...
                      <div className="text-right shrink-0">
                        ...
                        <div className="font-bold text-slate-200">${(item.price * item.count).toFixed(2)}</div>
                      </div>
                    </div>
                  ))}
                </div>
