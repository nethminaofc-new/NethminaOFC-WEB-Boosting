...
import { getAllOrders, updateOrderStatus, deleteOrder } from '../services/dbService';
import { StoredOrder, OrderStatus } from '../types';
import { Search, RefreshCw, Trash2, Download, Filter, ExternalLink, ChevronLeft, ChevronRight, Phone, ImageIcon, X } from 'lucide-react';
...
export const AdminDashboard: React.FC = () => {
  ...
  const exportCSV = () => {
    const headers = ['Order ID', 'Date', 'Customer ID', 'Contact', 'Platform', 'Service', 'Quantity', 'Link', 'Unit Price', 'Total', 'Status', 'PayMethod', 'SenderPhone', 'RefNo'];
                    ...
                    <div className="space-y-3">
                      {order.items.map((item, idx) => (
                        <div key={idx} className="bg-slate-950/50 p-3 rounded-lg border border-slate-800">
                          ...
                          <div className="flex items-center gap-2 text-xs text-blue-400 mb-1 overflow-hidden">
                             <ExternalLink size={10} className="shrink-0"/>
                             <a href={item.link} target="_blank" rel="noopener noreferrer" className="hover:underline truncate block">
                               {item.link}
                             </a>
                          </div>
